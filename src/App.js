  ...
  const login = async () => {
    ...
    try {
      ...
      if (res.data.status) {
        ...
        setStatus(false);
        addToast("Login Error :  Wrong AUID or Password", {
          appearance: "error",
          autoDismiss: true,
        });
      }
  ...
  const getUser = async () => {
    ...
    if (token) {
      try {
        ...
        console.log();
        addToast("GET USER DATA ERROR" + error, {
          appearance: "error",
          autoDismiss: true,
        });
      }
  ...
  return (
    <Container>
      ...
      <div style={{ border: "1px solid black", padding: 50 }}>
          ...
          {status && (
            <ButtonGroup aria-label="Basic example">
              <Button variant="light" disabled>
                {" "}
                Start Auto Class Join
              </Button>
              <Button
                variant={!starter ? "secondary" : "primary"}
                disabled={starter ? true : false}
                onClick={(e) => {
                  setStarter(true);
                }}
              >
        ...
        <h5>
          {userData?.session_data?.student_name &&
            "Hi," + userData?.session_data?.student_name}
        </h5>
        {userData?.session_data?.student_name && (
          <h5>
            FEE PAYMENT :{" "}
            {userData?.session_data?.due_status
              ? "Fees Not Paid "
              : "Fees Paid "}
          </h5>
        )}
        {userData?.session_data?.student_name && (
          <h5>
            ELIGIBLITY :{" "}
            {userData?.session_data?.eligible_status
              ? "Eligible to attend Classes"
              : "Not Eligible to attend Classes "}
          </h5>
        )}
        ...
        <p>
          <h5>DESCRIPTION:</h5>
          1.Use ALIVE AUID and Password to login
          <br />
          2.Login and Toggle switch to ON to start AutoAlive
          <br />
          3.It will automatically Join Ongoing Class every 10 minutes i.e even
          if teacher restarts the session you ll be able to join it within 10
          minutes of restart
          <br />
          4.Its still an apha version of the app so don't completly rely on this
          <br />
          5.Keep this tab running in chrome browser <br />
          6.Turn off auto sleep timer in your device settings <br />
          7.You ll have to select the join audio option!! Its just a webpage
          after all.
          <br />
        </p>

        <h3>
          Not Charging any thing for the service.. Any generous donation will
          get me a cup of coffee! ❤
        </h3>

        <div style={{ display: "flex", justifyContent: "space-evenly" }}>
          {" "}
          <img src="/payment.jpeg" width={400} />
        </div>
      </div>
    </Container>
  );
